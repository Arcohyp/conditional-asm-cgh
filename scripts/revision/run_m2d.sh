#!/usr/bin/env bash
# M2 step D: continue training the spatial-conditional SFO-window model for
# 24 more epochs (warm-start from the step-C best) to lift the comb-valley
...
echo "[$(date)] M2 step D: continue training 24 epochs from step-C best (GPU$GPU)" | tee -a $REV/logs/m2.log
$PY train_colorcond.py \
  ...
  --output-dir $OUT > $REV/logs/train_m2_cond_d.log 2>&1
rc=$?
echo "[$(date)] step D rc=$rc" | tee -a $REV/logs/m2.log
[ $rc -ne 0 ] && exit $rc

echo "[$(date)] M2 step D eval: ours (both conventions) (GPU$GPU)" | tee -a $REV/logs/m2.log
$PY $REV/eval_ours_m2.py $GPU > $REV/logs/eval_ours_m2_d.log 2>&1
rc=$?
echo "[$(date)] eval ours rc=$rc" | tee -a $REV/logs/m2.log

echo "[$(date)] N4 re-run: composited RGB reconstruction" | tee -a $REV/logs/m2.log
$PY $REV/make_rgb_composite.py $GPU > $REV/logs/rgb_composite_d.log 2>&1
rc=$?
echo "[$(date)] rgb composite rc=$rc" | tee -a $REV/logs/m2.log
echo "[$(date)] M2 STEP D ALL DONE" | tee -a $REV/logs/m2.log
